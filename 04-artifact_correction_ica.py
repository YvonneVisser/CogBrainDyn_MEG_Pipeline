...
def run_ica(subject, tsss=None):
    ...
    raw_fnames = op.join(meg_subject_dir, 'timelimit_%s_block01_maxfiltered.fif' % subject)
    print("  Loading runs")
    ...
    if tsss:
        ica_name = op.join(config.study_path, subject,
                           'run_concat-tsss_%d-ica.fif' % tsss)
    else:
        ica_name = op.join(config.study_path, subject, 'run_concat-ica.fif')
